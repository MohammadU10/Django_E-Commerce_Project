...
class Spec(models.Model):
    ...
    def __str__(self):
        return f'{self.pk}. {self.category_id.name} {self.name}'
...
class Item(models.Model):
    ...
    brand = models.ForeignKey(Brand, null=True, blank=True, default="N/A", on_delete=models.CASCADE)
    ...
    price = models.DecimalField(max_digits=8, decimal_places=2) # Field for the item price, 8 max digits & up to 2 decimal places
...
class ItemSpecValue(models.Model):
    ...
    def __str__(self):
        ...
        """
        return f'{self.pk}. {self.item} - {self.spec} - {self.value}'
